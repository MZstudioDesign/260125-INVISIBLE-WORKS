...
- **적용**: SimpleQuote, DetailedQuote 모두
  ...
  다음 단계:
  1. Sheet2 초기 데이터 설정: POST /api/config/init 호출
  2. .env에 GOOGLE_SHEET_CONFIG_TAB=설정 추가
  3. 실서비스 테스트

  현재 안 된 것
  1. 간단 세부견적서 헤더 푸터 고정하고, 항목 늘어나면 다음 페이지에 써지는 것 현재는 페이지가 그냥 수만 늘어나고 푸터를 침범해버림 페이지 분할도 안됨
  2. 영문일 경우 항목 도메인 1년~3년 안내 등 고객이 보는 부분도 영어로 바뀌어야 함
  3. 상기 다음 단계로 해야할 그것도 아직 준비 안됨
  4. 페이지 여러장이 될 경우 그 총 견적 금액을 마지막 페이지에 써야함
